...
raw_embedding_sum = tf.reduce_sum(tf.abs(ue_raw), axis=1)
...
default_ue = tf.tile(tf.expand_dims(default_ue, axis=0), [batch_size,1])

#import pdb; pdb.set_trace()
#default_ue = tf.expand_dims(default_ue, axis=0)
#default_ue = tf.tile(default_ue, [4,1])
#default_ue = tf.tile(default_ue, [batch_size, 1])
#default_ue = tf.tile(default_ue, [tf.shape(ue_raw)[0],1])
